...
def check_telaat(app, Uitlening, Artikel, User):
    with app.app_context():
        ...
        for uitlening in uitleningen:
            if uitlening.end_date < datetime.now().date() and uitlening.actief and uitlening.warning_sent == 0: #Als de uitlening te laat is en er nog geen waarschuwing is gegeven
                ...
                if conflictUitlening: #Als er een conflict is, zien of het artikel mogelijks vervangen kan worden door een ander exemplaar van het zelfde artikel dat niet gereserveerd is
                    ...
                    if vervangArtikel: #Als er een of meerdere mogelijke vervangartikels zijn
                        ...
                        db.session.commit()
                        print(f"Artikel {conflictUitlening.artikel.title} is niet beschikbaar voor volgende week voor gebruiker {conflictUitlening.user.first_name} {conflictUitlening.user.last_name}")
            elif uitlening.end_date == datetime.now().date() + timedelta(days=1) and uitlening.actief and uitlening.reminder_sent == 0: #Als de uitlening morgen moet worden ingeleverd en er nog geen herinnering is verstuurd
                ...
                uitlening.reminder_sent = 1
                db.session.commit()
            elif uitlening.start_date < datetime.now().date() and not uitlening.actief and uitlening.warning_sent == 0 and uitlening.return_date == None:
                artikel = Artikel.query.filter_by(id=uitlening.artikel_id).first()
        ...
        print("Te laat en reminder check uitgevoerd") #Bericht dat de check is uitgevoerd
        

def check_blacklist(app, User):
    with app.app_context():
        ...
        for user in users:
            ...
            elif user.blacklisted == 0 and user.warning >= 3:
                user.blacklisted = 1
                user.blacklist_end_date = datetime.now() + timedelta(days=90)
                msg = Message('Blacklist', recipients=[user.email])
                msg.body = f"Beste {user.first_name},\n\nU heeft 3 waarschuwingen gekregen en bent nu op de blacklist. U kunt voorlopig geen artikelen meer uitlenen.\n\nMet vriendelijke groeten,\n\nEHB Uitleendienst"
                mail.send(msg)
                db.session.commit()
                print(f"Gebruiker {user.first_name} {user.last_name} is op de blacklist")
        print("Blacklist check uitgevoerd")
